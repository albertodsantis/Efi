...
Default accent: `#C96F5B` (stored as fallback in `emptyState`)

The accent system is multi-modal — accents can be plain hex colors, gradients, conic presets, or retro themes. Each is stored as a string key in the database and resolved at runtime.

#### Accent types

| Type | Format | Example |
|---|---|---|
| Flat hex | `#RRGGBB` | `#6366F1` |
| Gradient | `gradient:<key>` | `gradient:dawn` |
| Conic | `conic:<key>` | `conic:tiktok` |
| Retro | `retro:<key>` | `retro:crt` |

#### CSS variables generated by `getAccentCssVariables(value)`

Returns 8 CSS variables applied to `document.documentElement`:

| Variable | Derivation |
|---|---|
| `--accent-color` | Representative hex (resolved from any preset type) |
| `--accent-gradient` | Full gradient CSS string, or hex for flat colors |
| `--accent-foreground` | White or dark slate, chosen by WCAG contrast ratio |
...
| `--accent-secondary` | Secondary color for conic accents (e.g. TikTok cyan), `transparent` otherwise |

`--accent-gradient` is used by `Button` and `IconButton` primary tones instead of `--accent-color` so gradients render correctly.

Foreground selection logic (`getAccessibleAccentForeground`):

- resolves the representative hex from any accent type
- computes WCAG contrast ratio against `#0f172a` (dark slate) and `#ffffff` (white)
- picks whichever yields the higher ratio

#### Surface theme overrides (`getSurfaceOverrides`)

Certain accent presets override surface color tokens in addition to accent variables. Applied via JS on each accent/theme change. Overrides the `--body-theme-bg` variable and potentially all surface/text/line tokens.

Presets with surface overrides:

| Preset | Description |
|---|---|
| `gradient:dawn` | Sunrise sky — warm peach/amber surfaces, dramatic gradient background |
| `gradient:instagram` | IG palette — soft purple/coral surfaces, violet gradient background |
| `retro:crt` | CRT terminal — deep amber-on-black surfaces, forces dark mode |
| `#74a12e` (Matcha) | Green-tinted surfaces, forest dark mode |

Non-overriding accents fall back to `getDefaultBodyBg()` — the standard radial accent + surface gradient.

#### CRT mode (`retro:crt`)

When the CRT accent is selected:

- `data-crt` attribute is set on `<html>`
- Forces dark theme (cannot be toggled to light while active)
- Surface tokens override to deep amber-on-black
- `html[data-crt]::after` renders scanlines + vignette overlay at `z-index: 9999`
- The body background is driven by the surface override, not the default `--body-theme-bg`

#### Current palette (12 options, user-selectable in Settings)

| Name | Value |
|---|---|
| Vintage TV | `retro:crt` |
| Dawn | `gradient:dawn` |
...
- retro:crt is the only accent that locks the theme to dark mode
...
Display/script typeface: `Allura` (imported from Google Fonts). Used directly via inline `fontFamily` style in Landing.tsx for decorative text — not via a CSS variable, not for UI copy.

`--font-playwrite: "Playwrite Ireland", cursive` is declared in `@theme` but not yet used in any component.
...
| Mobile nav tab buttons | `1rem` (16px) |
...
The body uses a `var(--body-theme-bg)` CSS variable rather than a hardcoded gradient. This variable is set dynamically by `getSurfaceOverrides()` in `accent.ts` on every accent/theme change.

Default backgrounds (for flat hex accents without surface overrides):
...
Accent presets with surface overrides (`gradient:dawn`, `gradient:instagram`, `retro:crt`, `#74a12e`) replace `--body-theme-bg` entirely with custom multi-layer gradients, and also override `--surface-*`, `--text-secondary`, and `--line-*` tokens. This means those presets create a fully themed environment, not just a color tint.
...
- sidebar: sticky, scrollable, contains profile avatar/name + vertical nav list; background `--surface-overlay`
- sidebar wrapper has `borderRight: 2px solid var(--accent-secondary, transparent)` — visible only for conic accents (e.g. TikTok)
- main area has `border-l [border-color:var(--line-soft)]` — always present as a soft separator
...
- fixed bottom navigation bar: 5 tabs, full-width, positioned with safe-area insets
- bottom nav: `--surface-card-strong` background, `backdrop-blur-2xl`, `borderTop: 3px solid var(--accent-secondary, var(--line-soft))` — uses accent-secondary for conic accents, falls back to `--line-soft`
- nav tab labels (`10px bold`) hidden below 480px viewport width; icons always visible
- pull-to-refresh: touch gesture (threshold 120px), CircleNotch spinner indicator; triggers `refreshAppData()`
...
| Prop | Type | Default | Notes |
|---|---|---|---|
| `tone` | `'neutral' \| 'accent' \| 'success' \| 'warning' \| 'danger' \| 'info' \| 'review'` | `'neutral'` | 7 tones |
| `className` | `string` | - | Additional classes |

Styling: `rounded-[0.8rem]`, 11px bold uppercase, `tracking-[0.12em]`, `px-3 py-1`. Accent tone uses inline styles with `--accent-soft-strong` bg and `--accent-color` text. Review tone uses violet palette (`bg-violet-50 text-violet-600`, dark: `bg-violet-500/15 text-violet-300`).
...
| Prop | Type | Notes |
|---|---|---|
| `icon` | `React.ElementType` | required, displayed at 20px in `--text-secondary` color |
...
User/partner avatar with image and initials fallback.

| Prop | Type | Default | Notes |
|---|---|---|---|
| `src` | `string` | - | Image URL; falls back to initials on error |
| `name` | `string` | required | Used to compute initials (first + last word) |
| `size` | `number` | `48` | Width and height in px |
| `className` | `string` | - | Additional classes |

Styling: `rounded-[1rem]`, muted surface fallback background, font size = `size * 0.38`.
...
Features: `backdrop-blur-md`, decorative radial gradient orbs (hardcoded warm/teal tones, not accent-aware), mobile sheet drag indicator, `z-[120]`. Below `sm:` (640px): children align to bottom with safe-area padding. At `sm:` and above: children center in viewport. Note: this breakpoint is 640px, not the 1024px app breakpoint.
...
### 8.5 EfisystemWidget (`components/EfisystemWidget.tsx`)

Gamification widget displaying the user's current level, points, and XP progress bar.

| Prop | Type | Notes |
|---|---|---|
| `efisystem` | `EfisystemSnapshot` | required — `{ totalPoints, currentLevel, unlockedBadges }` |
| `accentHex` | `string` | required — resolved accent hex for progress bar and badge button |
| `onOpenBadges` | `() => void` | optional — shows a "Placas" button when provided |

10 levels: Emergente → Explorer → Vibing → Máquina → Crack → Master → Élite → Authority → Ícono → Leyenda.

Level-up triggers confetti (`efi-confetti` custom event) and a toast. Badge unlocks show individual toasts. Progress bar uses `--accent-color` fill with accent-tinted ring shadow.
...
- modals align to bottom (sheet pattern) below `sm:` 640px, center in viewport at `sm:` and above — not tied to the 1024px app breakpoint
...
- checkbox/completion pop: `check-pop` keyframe, `0.22s ease-out` (scale 1 → 1.28 → 1)
...
| OverlayModal | `120` |
| Toaster | `200` |
| CRT scanline overlay (`html[data-crt]::after`) | `9999` |
...
When using UI/UX Pro Max or any other AI design tool:
